...
class SimpleRAGSystem:
    ...
    def search(self, query: str, n_results: int = 5) -> List[Dict]:
        """
        Search for relevant chunks with relevance scores.
            ...
            for i, (distance, idx) in enumerate(zip(distances[0], indices[0])):
                if idx < len(self.documents):
                    ...
                    formatted_results.append({
                        ...
                        'score': float(score),  # Add the relevance score
    ...
    def get_context_for_query(self, query: str, n_results: int = 5, max_context_length: int = 2000, min_score: float = 0.0) -> str:
        """
        ...
        for result in search_results:
            ...
            # Format the context piece with relevance score
            context_piece = f"[Source: {doc_id} | Relevance: {score:.2f}]\n{content}\n"
        ...
        if context_parts:
            header = f"Relevant context ({len(context_parts)} results"
            if filtered_count > 0:
                header += f", {filtered_count} filtered by threshold"
            header += "):\n\n"
            return header + "\n---\n".join(context_parts)
        else:
            if filtered_count > 0:
                return f"No relevant context found above threshold {min_score:.2f}. ({filtered_count} results were below threshold)"
            return "No relevant context found."
